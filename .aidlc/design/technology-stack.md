...
后端配置至少包括：`NODE_ENV`、`PORT`、`DATABASE_URL`、`JWT_ACCESS_SECRET`、access/refresh 有效期、`AWS_REGION`、`BEDROCK_AUTH_MODE`、`BEDROCK_MODEL_ID`，以及 API Key 模式下的 `BEDROCK_API_KEY`。可选 `REGISTRATION_STAFF_CODE`：设置后 operator/verifier 自助注册必须提供该码，用于避免任何人自助获取后台权限。仓库只提交 `.env.example` 占位符，不提交真实密码、token 或 AWS 凭据。
...
- 批量活动/核销、优惠券分享奖励、离线核销、支付、管理员自助注册（customer/operator/verifier 自助注册见 FR-011）。

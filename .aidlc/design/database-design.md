...
Seed 创建 `admin`、`operator`、`verifier`、`customer_a`、`customer_b`、`customer_c` 六个演示账号。`users` 表另接受自助注册写入的 `customer`、`operator`、`verifier` 账号（FR-011），密码同样只存 bcrypt 哈希；`admin` 角色只能来自 seed 或运维手动写入，注册接口不得创建。

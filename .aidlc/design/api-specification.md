...
- `/auth/register` 只接受 `customer`、`operator`、`verifier`；`admin` 一律 403，管理员账号只能由系统预置。角色以服务端写入值为准，不信任客户端后续声明。
- 本期不提供管理员用户 CRUD、批量操作、分享或离线核销 API。
...
| 方法 | 路径 | 权限 | 说明 | 映射 |
|---|---|---|---|---|
| POST | `/auth/register` | 公开 | 自助注册 customer/operator/verifier，成功即建立会话 | FR-011、US-013 |
...
```json
{"username":"new_customer","password":"Register123","displayName":"新用户","role":"customer","staffCode":"可选"}
```

约束：`username` 为 3-32 位字母、数字或下划线且全局唯一（大小写不敏感）；`password` 至少 8 位且同时包含字母和数字，只保存 bcrypt 哈希；`role` 仅接受 `customer`、`operator`、`verifier`；后端配置 `REGISTRATION_STAFF_CODE` 时，`operator` 与 `verifier` 必须提供匹配的 `staffCode`。

成功 `201` 返回与登录一致的会话结构（`accessToken`、`expiresIn`、`user`），并通过 Set-Cookie 下发 refresh token。

错误：`AUTH_ROLE_NOT_REGISTRABLE`(403，注册 admin)、`AUTH_STAFF_CODE_INVALID`(403)、`AUTH_USERNAME_TAKEN`(409)、`VALIDATION_ERROR`(400)。成功与被拒绝都写审计日志（`auth.register`），日志不含密码。
